# Hologram Module

## Overview

The hologram module enhances vanilla Minecraft's current system for holograms and backports all the improvements and vanilla features to Minecraft 1.7 players.

- Backports all vanilla minecraft hologram capabilities, found in 1.8+ to the 1.7 version of Lunar Client.
  - Added the interactivity that is provided in 1.8+ to the 1.7 version of Lunar Client.
- Adds improvements to enhance vanilla minecraft holograms.
  - Removes the need to do the any complex process to create holograms on 1.7 minecraft.
  - Custom hologram colors can be provided.

## Integration

### Sample Code

```java
public void displayHologramExample() {
    Collection<ApolloPlayer> viewers = Apollo.getPlayerManager().getPlayers();

    this.hologramModule.displayHologram(viewers, Hologram.builder()
        .id("welcome-hologram")
        .location(ApolloLocation.builder()
            .world("world")
            .z(5)
            .y(100)
            .z(0)
            .build())
        .lines(List.of(
            Component.builder()
                .content("Welcome to my server!")
                .color(Color.RED)
                .decorators(List.of(Component.TextDecorators.BOLD))
                .build(),
            Component.builder()
                .content("Type /help to get started!")
                .build()
        ))
        .build()
    );
}
```

### `Hologram` Options

`.id(String)` should include a unique identifier for the hologram.

```java
.id("welcome-hologram")
```

`.location(ApolloLocation)` is using the `ApolloLocation` builder to create the location. See the [locations page](/apollo/developers/utilities/locations) for more.

```java
.location(ApolloLocation.builder()
    .world("world")
    .z(5)
    .y(100)
    .z(0)
    .build()
) 
```

`.lines(Component)` should be a string, or a Apollo's builder in `Component` builder. See the [component builder page](/apollo/developers/utilities/components) for more.

```java
.lines(List.of(
    Component.builder()
        .content("Welcome to my server!")
        .color(Color.RED)
        .decorators(List.of(Component.TextDecorators.BOLD))
        .build(),
    Component.builder()
        .content("Type /help to get started!")
        .build()
    ))
```


### Removing a specific hologram for players

```java
public void removeHologramExample() {
    Collection<ApolloPlayer> viewers = Apollo.getPlayerManager().getPlayers();

    /*
     * Removes the hologram with the ID "welcome-hologram" for all
     * players inside of the viewers collection.
     */
    this.hologramModule.removeHologram(viewers, "welcome-hologram");
}
```

### Resetting all holograms for a player

```java
public void resetHologramsExample(Player viewer) {
    Optional<ApolloPlayer> apolloPlayerOpt = Apollo.getPlayerManager().getPlayer(viewer.getUniqueId());

    // Resetting every hologram
    apolloPlayerOpt.ifPresent(this.hologramModule::resetHolograms);
}
```
