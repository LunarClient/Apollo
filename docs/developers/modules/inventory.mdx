...
Inventory buttons displayed through the Apollo API with live content parts or a live tooltip are automatically resent while the `buttons.live-broadcast` option is enabled (disabled by default); no
