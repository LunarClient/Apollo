...
- Add wearable cosmetics to NPC entities
  ...
  - Copy the viewer's own equipped cosmetics onto an NPC
- Play emotes on NPC entities
  - Start, stop, and reset emotes on an NPC
...
**Stop an emote on an NPC**

```java
public void stopNpcEmoteExample(UUID npcUuid) {
...
`.id(int)` is the Lunar Client emote id.

```java
.id(56)
```

`.metadata(int)` selects a variant for emotes that support one, and is ignored otherwise (default `0`). For example, a coin flip emote may use `1` for heads and `2` for tails, while a rock paper scissors emote may use `1` for rock, `2` for paper, and `3` for scissors.
